...
;angular.module('app').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('templates/portfolio.tpl.html',
    "<div layout=row layout-align=\"center center\"><div class=\"portfolio-header md-whiteframe-z2 clearfix\" layout-padding flex-xs=99 flex-sm=95 flex-md=85 flex-gt-md=70><div><img id=me src=\"img/me.png\"></div><div class=\"name centered-margins\">John Anthony Rivera</div><div layout=row layout-wrap layout-align=\"space-around center\"><div class=contact-item>951-553-4461</div><div class=contact-item>johnanthony.dev@gmail.com</div><a class=contact-item href=https://github.com/coffeeexistence>Github</a> <a class=contact-item href=\"http://johnanthony-dev.com/blog/\">Blog</a></div></div></div><div layout=row class=portfolio-section flex=99 layout-align=\"center center\"><div class=portfolio-section layout=row layout-wrap layout-align=\"space-between start\" flex-xs=95 flex-sm=95 flex-md=85 flex-gt-md=70><div id=left class=portfolio-section flex-gt-xs=65 flex-xs=100 layout=column layout-wrap><div id=experience><h2>Technical Experience</h2><div class=indented><div ng-repeat=\"project in ctrl.technicalExperience\"><portfolio-project project=project></portfolio-project><md-divider ng-if=!$last></md-divider></div></div></div><div id=history><h2>Employment History</h2><md-list class=indented><div ng-repeat=\"job in ctrl.history\"><h3 class=header>{{job.name}}</h3><div class=subheader><span class=text-grey>{{job.title}}</span> <span class=text-grey>({{job.dateString}})</span></div><md-list-item ng-repeat=\"point in job.points\"><div class=list-item>{{point}}</div></md-list-item><md-divider ng-if=!$last></md-divider></div></md-list></div></div><div id=right class=skill-set flex-gt-xs=30 flex-xs=99 layout=column layout-align=\"start start\"><h2>Skill & Competencies</h2><div layout=column layout-align=\"start space-between\" layout=column layout-xs=row layout-wrap><div class=skill ng-repeat=\"tier1 in ctrl.skills\"><h3>{{tier1.title}}</h3><div class=sub-skill ng-repeat=\"tier2 in tier1.subSkills\"><h4>{{tier2.title}}</h4><div class=sub-skill ng-repeat=\"tier3 in tier2.subSkills\"><h5>{{tier3.title}}</h5></div></div></div></div></div></div></div>"
  );


  $templateCache.put('templates/project.tpl.html',
    "<div ng-mouseover=\"hover=true\" ng-mouseleave=\"hover=false\" ng-class=\"{'hovered': hover, 'md-whiteframe-z2 expanded': projectCtrl.expanded}\" class=\"noborder project-container group\"><div class=slight-indent><div layout=row layout-align=\"space-between center\"><div flex=75><h3 class=header>{{project.name}}</h3><div class=\"text-grey subheader\">{{project.description}}</div></div><div ng-click=projectCtrl.toggleExpansion(); class=\"text-grey show-more\"><md-button class=\"md-raised text-grey\" ng-if=!projectCtrl.expanded>more</md-button><md-button class=\"md-raised text-grey\" ng-if=projectCtrl.expanded>less</md-button></div></div><div class=list-item ng-repeat=\"point in project.points\"><div>{{point}}</div></div><md-divider ng-if=projectCtrl.expanded></md-divider><div ng-show=projectCtrl.expanded class=project-extra-content layout=row layout-xs=column layout-align=\"space-around center\" ng-click=\"\"><a ng-repeat=\"link in project.links\" ng-href={{link.url}} target=_blank class=text-grey>{{link.title}}</a></div><md-divider ng-if=\"project.element && projectCtrl.expanded\"></md-divider><div ng-if=\"project.element && projectCtrl.expanded\" bind-html-compile=project.element>{{project.element}}</div></div></div>"
  );
