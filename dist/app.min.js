...
;!function(a){"use strict";var b=a.module("angular-bind-html-compile",[]);b.directive("bindHtmlCompile",["$compile",function(a){return{restrict:"A",link:function(b,c,d){b.$watch(function(){return b.$eval(d.bindHtmlCompile)},function(e){c.html(e&&e.toString());var f=b;d.bindHtmlScope&&(f=b.$eval(d.bindHtmlScope)),a(c.contents())(f)})}}}])}(window.angular);;/*! angular-gist-embed 18-01-2015 */
!function(window,document){angular.module("gist-embed.config",[]).value("gist-embed.config",{debug:!0}),angular.module("gist-embed.directives",[]),angular.module("gist-embed.services",[]),angular.module("gist-embed",["gist-embed.config","gist-embed.directives","gist-embed.services"]),angular.module("gist-embed.directives").directive("compileGistEmbed",["$compile",function($compile){return function(scope,element,attrs){scope.$watch(function(scope){return scope.$eval(attrs.ngBindHtml)},function(value){element.html(value),$compile(element.contents())(scope)})}}]),angular.module("gist-embed.services").service("gistEmbed",["$document","$q","$timeout",function($document,$q,$timeout){var libUrl="//cdnjs.cloudflare.com/ajax/libs/gist-embed/2.0/gist-embed.min.js",loadSdkAsync=function(src){var deferred=$q.defer(),script=$document[0].createElement("script");return script.onload=script.onreadystatechange=function(e){$timeout(function(){deferred.resolve(e)})},script.onerror=function(e){$timeout(function(){deferred.reject(e)})},script.src=src,$document[0].body.appendChild(script),deferred.promise};this.init=function(){return loadSdkAsync(libUrl)}}]).run(["$rootScope","$timeout","gistEmbed",function($rootScope,$timeout,gistEmbed){gistEmbed.init().then(function(){$timeout(function(){angular.element(document).ready(function(){"function"==typeof angular.element(document).gist&&angular.element("[data-gist-id]").gist()})},300)}).then(function(){$rootScope.$on("$includeContentLoaded",function(){angular.element(document).ready(function(){"function"==typeof angular.element(document).gist&&angular.element("[data-gist-id]").gist()})})})}])}(window,document);;angular
...
;angular.module('app').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('templates/portfolio.tpl.html',
    "<md-content class=\"animated fadeIn\" style=\"height: 100%; width:100%\"><div layout=row flex=99 layout-align=\"center center\"><div class=\"portfolio-header md-whiteframe-z1 clearfix\" layout=row layout-align=\"center center\" layout-wrap flex-xs=99 flex-sm=95 flex-md=85 flex-gt-md=70><div flex=99 flex-gt-sm=99 class=\"secondary-grey-lighten-1 name\">John <span class=secondary-grey-lighten-4>Anthony</span> Rivera<br><what-am-i></what-am-i></div><img flex=99 flex-gt-sm=99 href id=me src=img/me2.png></div></div><div layout=row flex=99 layout-align=\"center center\"><div class=tab-selector layout=row layout-align=\"space-around center\" flex-xs=99 flex-sm=95 flex-md=85 flex-gt-md=70><div ng-repeat=\"tab in tabs.tabs track by $index\" class=tab ng-class=\"{'tab-selected': tabs.selected($index), 'tab-unselected': !tabs.selected($index)}\"><a flex href ng-click=tabs.set($index)>{{tabs.tabs[$index]}}</a></div></div></div><div ng-show=\"tabs.on('Contact')\" layout=column layout-align=\"center center\" flex=99><div id=contacts-container class=\"animated fadeInRight\" layout=column layout-align=\"start start\" flex-xs=99 flex-sm=95 flex-md=85 flex-gt-md=70><div class=contact-item layout=row layout-align=\"center center\"><i class=\"material-icons md-48\">phone_iphone</i><div>951-553-4461</div></div><div class=contact-item layout=row layout-align=\"center center\"><i class=\"material-icons md-48\">mail_outline</i><div>johnanthony.dev@gmail.com</div></div><div class=contact-item layout=row layout-align=\"center center\"><i class=\"material-icons md-48\">code</i><div>Github: coffeeexistence</div></div></div></div><div ng-show=\"tabs.on('Code')\" layout=row layout-align=\"center center\" layout-padding><div layout=column flex-xs=99 flex-sm=95 flex-md=85 flex-gt-md=70><div layout=row layout-align=\"center center\"><div flex><hr class=gist-title-hr></div><h2 style=\"margin: 1em; padding-left: 0px\">The Gist Of It</h2><div flex><hr class=gist-title-hr></div></div><div class=gist-description layout=row layout-align=\"start start\" layout-wrap><div flex=99 flex-gt-sm=35><div layout=row class=gist-nav layout-align=\"start start\" style=\"margin-bottom: 1em\"><i class=\"material-icons md-36\" ng-click=gists.previous();>chevron_left</i> <i class=\"material-icons md-36\" ng-click=gists.next();>chevron_right</i></div><div class=gist-title>{{gists.current().title}}</div><h3 class=x-thin>Problem</h3><p class=\"secondary-grey-lighten-2 x-thin\">{{gists.current().problem}}</p><h3 class=x-thin>Solution</h3><p class=\"secondary-grey-lighten-1 x-thin\">{{gists.current().solution}}</p></div><div id=gist-container flex=99 flex-gt-sm=65></div></div><div layout=row layout-align=\"center center\"><div flex><hr class=gist-title-hr></div></div></div></div><div ng-show=\"tabs.on('Projects & Experience')\" layout=row class=portfolio-section flex=99 layout-align=\"space-around start\"><div class=\"portfolio-section animated fadeInRight\" layout=row layout-align=\"space-between start\" layout-wrap flex-xs=99 flex-sm=95 flex-md=85 flex-gt-md=70><div id=left class=\"portfolio-section primary-color\" flex-gt-md=75 flex=99 layout=column layout-wrap><div id=experience><h2>Projects</h2><div class=indented><div ng-repeat=\"project in ctrl.technicalExperience\"><portfolio-project project=project></portfolio-project><md-divider ng-if=!$last></md-divider></div></div></div><div id=history style=\"margin-top: 2em\"><h2 class=primary-color>History</h2><md-list class=\"indented secondary-grey-lighten-3\"><div ng-repeat=\"job in ctrl.history\"><h3 class=header>{{job.name}}</h3><div class=subheader><span class=text-grey>{{job.title}}</span> <span class=text-grey>({{job.dateString}})</span></div><md-list-item ng-repeat=\"point in job.points\"><div class=list-item>{{point}}</div></md-list-item><md-divider ng-if=!$last></md-divider></div></md-list></div></div><div id=right class=\"skill-set secondary-grey\" flex-gt-md=20 flex=99 layout=column layout-fill layout-align=\"start start\"><h2>Skills</h2><div layout=row layout-align=\"start start\" layout-align-gt-md=\"start start\" layout-fill layout-wrap><div class=skill ng-repeat=\"tier1 in ctrl.skills\"><h3>{{tier1.title}}</h3><div class=sub-skill ng-repeat=\"tier2 in tier1.subSkills\"><h4>{{tier2.title}}</h4><div class=sub-skill ng-repeat=\"tier3 in tier2.subSkills\"><h5>{{tier3.title}}</h5></div></div></div></div></div></div></div></md-content>"
  );


  $templateCache.put('templates/project.tpl.html',
    "<div ng-class=\"{'expanded md-whiteframe-z1': expand.state}\" class=\"project-container group\"><div class=slight-indent layout=row layout-align-xs=\"center center\" layout-align=\"start center\" layout-wrap><div flex-gt-xs=10 flex-xs=99><img class=project-icon ng-src=\"{{project.icon}}\"></div><div flex-xs=99 flex-gt-xs=70 layout=row layout-align=\"space-between start\"><div><h3 class=\"project-title header thin\">{{project.name}}</h3><div class=\"text-grey subheader\">{{project.description}}</div></div></div><div flex-xs=99 flex-gt-xs=20 class=text-grey layout=column layout-fill layout-align=\"end start\" style=\"height: 100%\"><div class=show-more><ui-button ng-click=showAdvanced($event) class=\"md-raised text-grey\">{{expand.text}}</ui-button></div></div></div><div layout=column ng-show=expand.state layout-align=\"center space-around\"><md-divider></md-divider><md-divider ng-if=project.element></md-divider><div ng-if=project.element bind-html-compile=project.element>{{project.element}}</div></div></div>"
  );


  $templateCache.put('templates/projectDialog.tpl.html',
    "<md-dialog aria-label=\"Mango (Fruit)\" ng-cloak><form><div flex=99 class=project-dialog-header layout=row layout-align=\"center center\"><h2>{{project.name}}</h2></div><md-divider></md-divider><md-dialog-content><div class=md-dialog-content><img class=\"project-banner md-whiteframe-z1\" ng-if=project.banner ng-src=\"{{project.banner}}\"><div layout=row layout-align=\"center center\" layout-wrap><div flex-xs=99 flex-gt-xs=35 layout=column layout-align=\"center center\" ng-if=project.mockups><img flex class=\"project-mockup animated\" ng-src={{mockup.current}} ng-class=\"{fadeInLeft: mockup.show, fadeOut: !mockup.show, }\"></div><div class=project-points flex-xs=99 flex-gt-xs=bulletPointWidth layout=column layout-align=\"start start\"><div class=list-item ng-repeat=\"point in project.points\"><div class=project-point>&bull; {{point}}</div></div><div ng-if=project.element bind-html-compile=project.element>{{project.element}}</div></div></div></div></md-dialog-content><md-dialog-actions class=project-extra-content layout=row layout-xs=column layout-align=\"space-around center\"><a ng-repeat=\"link in project.links\" ng-href={{link.url}} target=_blank class=text-grey>{{link.title}}</a><ui-button ng-click=closeDialog()>Close</ui-button></md-dialog-actions></form></md-dialog>"
  );


  $templateCache.put('templates/repo_display.tpl.html',
    "Hello!<div style=margin:2em ng-repeat=\"file in browser.files\"><pre><code data-language=javascript>{{file.text}}</code></pre></div>"
  );


  $templateCache.put('templates/tictac.tpl.html',
    "<div layout=column layout-align=\"center center\" class=game-container><div layout-align=\"center center\" layout=column><h2 style=\"font-weight: 300\">Tic Tac Toe with AI</h2><md-radio-group ng-model=difficulty><p>Difficulty:</p><md-radio-button value=0>Too Easy</md-radio-button><md-radio-button value=4>Literally Impossible</md-radio-button></md-radio-group><md-button ng-click=TicTacCtrl.init()>New Game</md-button><p ng-show=loading>Waking up the sleepy server</p><md-progress-linear ng-show=loading md-mode=indeterminate></md-progress-linear></div><div ng-if=loaded layout=column layout-align=\"center center\" layout-wrap><h3 ng-hide=gameStatus().over>{{message}}</h3><h2 ng-show=gameStatus().over>{{gameOverMessage()}}</h2><h4 ng-show=gameOverAlert>{{gameOverAlert}}</h4><hr><table class=game-board border=1><tr><td game-slot class=game-slot index=1></td><td game-slot class=game-slot index=2></td><td game-slot class=game-slot index=3></td></tr><tr><td game-slot class=game-slot index=4></td><td game-slot class=game-slot index=5></td><td game-slot class=game-slot index=6></td></tr><tr><td game-slot class=game-slot index=7></td><td game-slot class=game-slot index=8></td><td game-slot class=game-slot index=9></td></tr></table><br></div></div>"
  );

}]);
;function ChallengesController($sce){
	var ctrl = this;
