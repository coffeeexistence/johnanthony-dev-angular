...
;angular.module('app').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('templates/project.tpl.html',
    "<div ng-click=projectCtrl.toggleExpansion(); ng-mouseover=\"hover=true\" ng-mouseleave=\"hover=false\" ng-class=\"{'hovered': hover, 'md-whiteframe-z2 expanded': projectCtrl.expanded}\" class=\"noborder project-container\"><div class=slight-indent><div layout=row layout-align=\"space-between center\"><h3>{{project.name}}</h3><div ng-show=hover class=hover-text><span ng-show=!projectCtrl.expanded>(Click to expand)</span> <span ng-show=projectCtrl.expanded>(Click to collapse)</span></div></div><div class=list-item ng-repeat=\"point in project.points\"><div>{{point}}</div></div><md-divider ng-if=projectCtrl.expanded></md-divider><div ng-show=projectCtrl.expanded class=project-extra-content layout=row layout-xs=column layout-align=\"space-around center\" ng-click=\"\"><a ng-repeat=\"link in project.links\" ng-href={{link.url}}>{{link.title}}</a></div></div></div>"
  );


  $templateCache.put('templates/tictac.tpl.html',
    "<div class=game-container><h2>I AM TIC TAC</h2>Difficulty:<input ng-model=\"difficulty\"><br><button ng-click=TicTacCtrl.init()>Start Game</button><div ng-if=loaded layout=column layout-align=\"center center\" layout-wrap><h3>Your session id is: {{game.id}}</h3><h3>Current Player: {{currentPlayer().name}}</h3><hr><table class=game-board border=1><tr><td game-slot index=1></td><td game-slot index=2></td><td game-slot index=3></td></tr><tr><td game-slot index=4></td><td game-slot index=5></td><td game-slot index=6></td></tr><tr><td game-slot index=7></td><td game-slot index=8></td><td game-slot index=9></td></tr></table><button ng-click=TicTacCtrl.getAiMove()>Get AI Move</button><br></div>{{game}}</div>"
  );
